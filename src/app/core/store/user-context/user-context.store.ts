...
export const UserContextStore = signalStore(
  { providedIn: 'root' },
  withEntities<UserContext>(),
  withMethods((store) => ({
    selectEntity(id: string): UserContext {
      return store.entityMap()[id];
    },
    selectFirst(queryParams: QueryParams, queryOptions: QueryOptions): UserContext {
      const entities = selectEntities<UserContext>(store.entityMap(), queryParams, queryOptions);
    ...
    selectEntities(queryParams: QueryParams, queryOptions: QueryOptions): UserContext[] {
      return selectEntities<UserContext>(store.entityMap(), queryParams, queryOptions);
    },
  })),
  withMethods((store, cachedLoads = inject(CachedListenersService), db = inject(FirebaseService)) => ({
    ...
    async stream(
      ...
      queryOptions: QueryOptions,
      injector,
      streamQueries?: (userContext: UserContext) => EntityStreamQuery<AnyEntity>[],
      options?: {
      ...
      try {
        ...
        await cachedLoads.processStreamQuery('userContexts', store, queryParams, queryOptions, injector, streamQueries, options);
      } catch (e) {
        console.error(e);
    ...
    async load(
      queryParams: QueryParams,
      queryOptions: QueryOptions,
      loadQueries?: (userContext: UserContext) => EntityLoadQuery<AnyEntity>[],
      options?: {
      ...
      try {
        ...
        if (!cachedLoads.isStreamForQuery('userContexts', queryParams, queryOptions)) {
          // get the last load time and next load time
          const lastLoadTime = cachedLoads.getLastLoadTime('userContexts', queryParams, queryOptions);
          ...
          const entitiesDelta: UserContext[] = await db.getEntities(
            'userContexts',
            queryParamsWithTime,
            queryOptions,
          );
          ...
          if (lastLoadTime === -1 || (queryOptions && Object.keys(queryOptions).length > 0)) {
            const entitiesDeletedDelta: UserContext[] = await db.getEntities(
              'userContexts',
          ...
          cachedLoads.setLastLoadTime('userContexts', queryParams, queryOptions, newLoadTime);
        }
        ...
        if (loadQueries) {
          ...
          await processLoadQueries<UserContext>(allQueriedEntities, loadQueries);
        }
      } catch (e) {
        console.error(e);
        throw e;
    ...
    async count(
      ...
      try {
        ...
        const querySnapshot = await db.count('userContexts', queryParamsNotDeleted, queryOptions);
    ...
    async aggregate<S extends AggregateSpec>(
      ...
      try {
        ...
        const querySnapshot = await db.aggregate<S>(
          'userContexts', queryParamsNotDeleted, queryOptions, aggregateSpec,
        );
    ...
    async increment(id: string, field: string, delta: number, options?: {
      ...
      if (options?.optimistic) {
        try {
          ...
          await db.incrementEntityField('userContexts', id, field, delta);
        } catch (e) {
    ...
    async add(entityOpId: UserContext | Omit<UserContext, '__id'>, options?: {
      loading?: WritableSignal<boolean>,
      optimistic?: boolean
    }) {
      // Create an __id if one was not provided
      const entity: UserContext = Object.assign({}, entityOpId, {
      ...
      if (options?.optimistic) {
        ...
        try {
          ...
          await db.addEntity('userContexts', entity);
        } catch (e) {
          // undo ngrx store value
          if (currEntityValue) {
            patchState(store, setEntity(currEntityValue, { idKey: '__id' }));
    ...
    async update(id: string, changes: Partial<UserContext>, options?: {
      loading?: WritableSignal<boolean>,
      optimistic?: boolean
    }) {
      if (options?.optimistic) {
        ...
        try {
          // optimistically set then query backend
          patchState(store, updateEntity({ id, changes }));
          await db.updateEntity('userContexts', id, changes);
        } catch (e) {
          // undo ngrx store value
          if (currEntityValue) {
            patchState(store, setEntity(currEntityValue, { idKey: '__id' }));
    ...
    async remove(id: string, options?: {
      ...
      if (options?.optimistic) {
        ...
        try {
          ...
          await db.removeEntity('userContexts', id);
        } catch (e) {
          // undo ngrx store value
          if (currEntityValue) {
            patchState(store, setEntity(currEntityValue, { idKey: '__id' }));
